# Hi, I'm Aditya 👋

I'm a **Backend Developer** who loves building high-performance systems and developer tools with **Golang**.  
Currently contributing to **open-source projects** like [GitHub CLI](https://github.com/cli/cli) and **CNCF Layer5**.

- ⚙️ I love working with **Go**, **Docker**, **Kubernetes**, and **CLI tools**
- 💡 Creator of [ZevFocus](https://zevfocus.github.io/ZevFocus/) — a productivity tracker used by 500+ developers
- 🧠 Currently exploring **concurrency patterns** and **network programming** in Go
- 💬 Ask me about **backend architecture**, **APIs**, or **CLI tooling**
- 🛠 I use **nvim**, **i3wm**, and **tmux** (btw 😄)

📫 Reach me at: [mishraaditya675@gmail.com](mailto:mishraaditya675@gmail.com)

![Aditya's GitHub Stats](https://github-readme-stats.vercel.app/api?username=2003Aditya&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=2003Aditya&layout=compact&theme=tokyonight)

### 🚀 Featured Projects

- [**ZevFocus**](https://github.com/zevfocus) — CLI productivity tracker built in Go; 500+ installs  
- [**Zenith**](https://github.com/2003Aditya/teamzenith) — Offline mesh-based chat & location sharing app  
- [**GitHub CLI Contributions**](https://github.com/cli/cli/pull/12053) — Fixed `--interval` flag docs in `gh pr checks`

### 🌐 Connect with me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/aditya-mishra-670083215/)
[![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/2003Aditya)

[![Aditya's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=2003Aditya&theme=tokyo-night)](https://github.com/ashutosh00710/github-readme-activity-graph)

![snake gif](https://github.com/2003Aditya/2003Aditya/blob/output/github-contribution-grid-snake.svg)
